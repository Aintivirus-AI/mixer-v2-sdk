...
function refreshSeasonAssetFromContract(
  ...
  let oldTotalStaked = sa.totalStaked
  let oldTotalReward = sa.totalReward
  ...
  season.totalStaked = season.totalStaked.minus(oldTotalStaked).plus(sa.totalStaked)
  season.totalReward = season.totalReward.minus(oldTotalReward).plus(sa.totalReward)
  season.save()
}
...
export function handleRewardAdded(event: RewardAddedEvent): void {
  ...
  let p = protocol()
  p.stakingAddress = event.address
  p.totalRewardsAddedAllTime = p.totalRewardsAddedAllTime.plus(event.params.amount)
  p.updatedBlockNumber = event.block.number
  p.updatedBlockTimestamp = event.block.timestamp
  p.updatedTransactionHash = event.transaction.hash
  p.save()
...
export function handleStaked(event: StakedEvent): void {
  ...
  let ss = getOrCreateStakerSeason(event.params.staker, event.params.seasonId)
  ss.totalStaked = ss.totalStaked.plus(event.params.amount)
  ss.save()

  let p = protocol()
  p.stakingAddress = event.address
  p.totalStaked = p.totalStaked.plus(event.params.amount)
  p.totalStakedAllTime = p.totalStakedAllTime.plus(event.params.amount)
  p.updatedBlockNumber = event.block.number
  p.updatedBlockTimestamp = event.block.timestamp
  p.updatedTransactionHash = event.transaction.hash
  p.save()
...
export function handleUnstaked(event: UnstakedEvent): void {
  ...
  let ss = getOrCreateStakerSeason(event.params.staker, inferredSeasonId)
  if (ss.totalStaked.ge(event.params.amount)) {
    ss.totalStaked = ss.totalStaked.minus(event.params.amount)
  } else {
    ss.totalStaked = BigInt.zero()
  }
  ss.save()
